// ============================================================
// FILE: multiplier_rca_buggy.v — Buggy multiplier for error demo
// ============================================================
module dff_b (output reg q, input wire d, input wire clk);
    always @(posedge clk) q <= d;
endmodule

module multiplier_rca_buggy (
    ...
    dff_b dff_x0(.q(xr[0]),.d(x[0]),.clk(clk)); dff_b dff_x1(.q(xr[1]),.d(x[1]),.clk(clk));
    dff_b dff_x2(.q(xr[2]),.d(x[2]),.clk(clk)); dff_b dff_x3(.q(xr[3]),.d(x[3]),.clk(clk));
    dff_b dff_y0(.q(yr[0]),.d(y[0]),.clk(clk)); dff_b dff_y1(.q(yr[1]),.d(y[1]),.clk(clk));
    dff_b dff_y2(.q(yr[2]),.d(y[2]),.clk(clk)); dff_b dff_y3(.q(yr[3]),.d(y[3]),.clk(clk));
    dff_b dff_y4(.q(yr[4]),.d(y[4]),.clk(clk)); dff_b dff_y5(.q(yr[5]),.d(y[5]),.clk(clk));
    dff_b dff_y6(.q(yr[6]),.d(y[6]),.clk(clk)); dff_b dff_y7(.q(yr[7]),.d(y[7]),.clk(clk));
    ...
    rca_4bit_buggy add1_lo(.sum(s1_lo),.cout(c1_lo),.a(pp0[4:1]),       .b(pp1[3:0]),.cin(1'b0));
    rca_4bit_buggy add1_hi(.sum(s1_hi),.cout(c1_hi),.a({1'b0,pp0[7:5]}),.b(pp1[7:4]),.cin(c1_lo));
    wire [9:0] acc1;
    ...
    rca_4bit_buggy add2_lo(.sum(s2_lo),.cout(c2_lo),.a(acc1[5:2]),.b(pp2[3:0]),.cin(1'b0));
    rca_4bit_buggy add2_hi(.sum(s2_hi),.cout(c2_hi),.a(acc1[9:6]),.b(pp2[7:4]),.cin(c2_lo));
    wire [10:0] acc2;
    ...
    rca_4bit_buggy add3_lo(.sum(s3_lo),.cout(c3_lo),.a(acc2[6:3]), .b(pp3[3:0]),.cin(1'b0));
    rca_4bit_buggy add3_hi(.sum(s3_hi),.cout(c3_hi),.a(acc2[10:7]),.b(pp3[7:4]),.cin(c3_lo));
    ...
    dff_b dff_r0 (.q(res[0]), .d(result_comb[0]), .clk(clk));
    dff_b dff_r1 (.q(res[1]), .d(result_comb[1]), .clk(clk));
    dff_b dff_r2 (.q(res[2]), .d(result_comb[2]), .clk(clk));
    dff_b dff_r3 (.q(res[3]), .d(result_comb[3]), .clk(clk));
    dff_b dff_r4 (.q(res[4]), .d(result_comb[4]), .clk(clk));
    dff_b dff_r5 (.q(res[5]), .d(result_comb[5]), .clk(clk));
    dff_b dff_r6 (.q(res[6]), .d(result_comb[6]), .clk(clk));
    dff_b dff_r7 (.q(res[7]), .d(result_comb[7]), .clk(clk));
    dff_b dff_r8 (.q(res[8]), .d(result_comb[8]), .clk(clk));
    dff_b dff_r9 (.q(res[9]), .d(result_comb[9]), .clk(clk));
    dff_b dff_r10(.q(res[10]),.d(result_comb[10]),.clk(clk));
    dff_b dff_r11(.q(res[11]),.d(result_comb[11]),.clk(clk));
endmodule
