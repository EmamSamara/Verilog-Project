// ============================================================
// FILE: multiplier_rca.v  —  Stage 1: RCA Multiplier
// 4-bit x 8-bit unsigned multiplier, result = 12 bits
// All I/O registered through D flip-flops
// Max latency ≈ 251 ns  |  Max Fclk ≈ 3.98 MHz
// ============================================================
module multiplier_rca (
    ...
    dff dff_x0(.q(xr[0]),.d(x[0]),.clk(clk)); dff dff_x1(.q(xr[1]),.d(x[1]),.clk(clk));
    dff dff_x2(.q(xr[2]),.d(x[2]),.clk(clk)); dff dff_x3(.q(xr[3]),.d(x[3]),.clk(clk));
    dff dff_y0(.q(yr[0]),.d(y[0]),.clk(clk)); dff dff_y1(.q(yr[1]),.d(y[1]),.clk(clk));
    dff dff_y2(.q(yr[2]),.d(y[2]),.clk(clk)); dff dff_y3(.q(yr[3]),.d(y[3]),.clk(clk));
    dff dff_y4(.q(yr[4]),.d(y[4]),.clk(clk)); dff dff_y5(.q(yr[5]),.d(y[5]),.clk(clk));
    dff dff_y6(.q(yr[6]),.d(y[6]),.clk(clk)); dff dff_y7(.q(yr[7]),.d(y[7]),.clk(clk));

    // Partial Products: pp[k][i] = yr[i] AND xr[k]
    ...
    // Step 1: acc1 = PP0 + (PP1 << 1)
    wire [3:0] s1_lo; wire c1_lo; wire [3:0] s1_hi; wire c1_hi;
    rca_4bit add1_lo(.sum(s1_lo),.cout(c1_lo),.a(pp0[4:1]),      .b(pp1[3:0]),.cin(1'b0));
    rca_4bit add1_hi(.sum(s1_hi),.cout(c1_hi),.a({1'b0,pp0[7:5]}),.b(pp1[7:4]),.cin(c1_lo));
    wire [9:0] acc1;
    assign acc1[0]=pp0[0]; assign acc1[4:1]=s1_lo; assign acc1[8:5]=s1_hi; assign acc1[9]=c1_hi;

    // Step 2: acc2 = acc1 + (PP2 << 2)
    wire [3:0] s2_lo; wire c2_lo; wire [3:0] s2_hi; wire c2_hi;
    rca_4bit add2_lo(.sum(s2_lo),.cout(c2_lo),.a(acc1[5:2]),.b(pp2[3:0]),.cin(1'b0));
    rca_4bit add2_hi(.sum(s2_hi),.cout(c2_hi),.a(acc1[9:6]),.b(pp2[7:4]),.cin(c2_lo));
    wire [10:0] acc2;
    assign acc2[1:0]=acc1[1:0]; assign acc2[5:2]=s2_lo; assign acc2[9:6]=s2_hi; assign acc2[10]=c2_hi;

    // Step 3: result = acc2 + (PP3 << 3)
    wire [3:0] s3_lo; wire c3_lo; wire [3:0] s3_hi; wire c3_hi;
    rca_4bit add3_lo(.sum(s3_lo),.cout(c3_lo),.a(acc2[6:3]), .b(pp3[3:0]),.cin(1'b0));
    rca_4bit add3_hi(.sum(s3_hi),.cout(c3_hi),.a(acc2[10:7]),.b(pp3[7:4]),.cin(c3_lo));
    ...
    // Registered outputs
    dff dff_r0 (.q(res[0]), .d(result_comb[0]), .clk(clk));
    dff dff_r1 (.q(res[1]), .d(result_comb[1]), .clk(clk));
    dff dff_r2 (.q(res[2]), .d(result_comb[2]), .clk(clk));
    dff dff_r3 (.q(res[3]), .d(result_comb[3]), .clk(clk));
    dff dff_r4 (.q(res[4]), .d(result_comb[4]), .clk(clk));
    dff dff_r5 (.q(res[5]), .d(result_comb[5]), .clk(clk));
    dff dff_r6 (.q(res[6]), .d(result_comb[6]), .clk(clk));
    dff dff_r7 (.q(res[7]), .d(result_comb[7]), .clk(clk));
    dff dff_r8 (.q(res[8]), .d(result_comb[8]), .clk(clk));
    dff dff_r9 (.q(res[9]), .d(result_comb[9]), .clk(clk));
    dff dff_r10(.q(res[10]),.d(result_comb[10]),.clk(clk));
    dff dff_r11(.q(res[11]),.d(result_comb[11]),.clk(clk));
endmodule
